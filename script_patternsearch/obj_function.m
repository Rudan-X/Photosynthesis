...
M = ones(length(model_d.sim_ind),length(model_d.sim_ind));
M(2:40,2:40)=zeros(39);
...
for cond=1:model_d.ncond
    ...
    options=odeset('Mass',M,'NonNegative',1:length(model_d.sim_ind),'RelTol',1e-3,'AbsTol',1e-3,'Events',overfunc);% ,'InitialStep',0.001
%
