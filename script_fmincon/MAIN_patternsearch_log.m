...
check=load('patternsearch_result_1_5day.mat');
init_sol_log=check.x;

%%
tic
options = optimoptions('fmincon','Display','iter','UseParallel',true,'MaxFunctionEvaluations',200);

[x,fval,exitflag,output] = fmincon(fun,init_sol_log,A,b,Aeq,beq,lb,ub,nonlcon,options);

save('fmincon_result_3days.mat','x','fval','exitflag','output')
toc