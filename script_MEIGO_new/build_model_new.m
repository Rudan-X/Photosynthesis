...
model=addReaction(model,'transk2','S7P + GAP[s] <=> X5P + R5P'); cat(length(model.rxns))={'transk'}; % Transketolase(tk) 2.2.1.1	S7P+GAP<->Ri5P+X5P
...
model=addReaction(model,'fbpasec','F6P[c] + Pi[c] <=> FBP[c] + H2O[c]'); cat(length(model.rxns))={'fbpase_c'}; % FBPase(fbpase) 3.1.3.11	FBP<->F6P+OP
...
model=addReaction(model,'ugpase','G1P[c] + UTP <=> 2 Pi[c] + UDPG'); cat(length(model.rxns))={'ugpase'}; % UDP glucose pyrohpsphorylase(ugpase) 2.7.7.9: G1P+UTP -->2Pi +UDPG 
